...
crc_sample_prefix = Namespace("https://crc1625.mdi.ruhr-uni-bochum.de/materials_library/")
...
def get_label(entity_iri: str, store:RDFDatastore):
    ...
    return store.launch_query(query).json()["results"]["bindings"][0]["label"]["value"]


def get_activity_type(entity_iri: str, store:RDFDatastore):
    """
    ...
    result = store.launch_query(query).json()["results"]["bindings"]
    if len(result) > 0:
            ...
            if p in workflow_model_step_iri_to_config:
                match workflow_model_step_iri_to_config[p]:
                    ...
                    case "required_activities":
                        workflow_step.required_activities.append(iri_to_activity[get_activity_type(o, store)])
                    case _:
...
def get_first_handover_group(object_id: int, store: RDFDatastore) -> str:
    ...
    """
    if isinstance(store, VirtuosoRDFDatastore):
    ...
    result = store.launch_query(query)
    if len(result.json()["results"]["bindings"]) == 0:
        raise RuntimeError(f"No initial handover group found for sample {object_id}")

    return result.json()["results"]["bindings"][0]["first_handover_group"]["value"]


def get_next_handover_group(current_handover_group_iri: str, store: RDFDatastore) -> str | None:
    """
    ...
    return result.json()["results"]["bindings"][0]["next_handover_group"]["value"]
...
def get_next_validation_steps(workflow_model: WorkflowModel,
                              workflow_instance : WorkflowInstance,
                              current_workflow_step: WorkflowModelStep,
                              current_object_id: int,
                              target_node: str,
                              store: RDFDatastore) -> list[tuple[WorkflowModelStep, str, int, str]]:
    """
    ...
    if len(current_workflow_step.next_steps) > 0:
        ...
        for new_object_id in workflow_instance.step_assignments[next_step_name]:
            if current_object_id == new_object_id: # The next target node is the next handover group for the target node
                new_target_node = get_next_handover_group(target_node, store)
                ...
                next_steps.append((workflow_model.workflow_model_steps[next_step_name], next_step_name, new_object_id, get_first_handover_group(new_object_id, store)))
...
def generate_SHACL_shapes_for_workflow(workflow_model: WorkflowModel,
                                       workflow_instance: WorkflowInstance,
                                       store: RDFDatastore) -> list[tuple[WorkflowModelStep, str, int, str, str]]:
    """
    ...
    for object_id in workflow_instance.step_assignments[workflow_model.workflow_model_options.initial_step_name]:
        first_handover_group = get_first_handover_group(object_id, store)
    ...
    while len(steps_to_parse) > 0:
        ...
        steps_to_parse.extend(get_next_validation_steps(workflow_model, workflow_instance, workflow_step, object_id, target_node, store))

    return steps_to_validate


def get_ttl_for_sample(object_id: str, store: RDFDatastore):
    """
...
def validate_SHACL_rules(steps_to_validate: list[(WorkflowModelStep, str, str)],
                         store: RDFDatastore) -> list[(WorkflowModelStep, int, str, str, bool, str)]:
    ...
    for (workflow_step, workflow_step_name, object_id, target_node, shacl_rules) in steps_to_validate:
        ...
        if object_id not in sample_ttls:
            sample_ttls[object_id] = get_ttl_for_sample(object_id, store)
