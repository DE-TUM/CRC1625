...
async def add_edge_action(step_name: str | None,
    ...
    get_state().current_workflow_instance.step_assignments[step_name].append(int(object_id))
...
async def remove_edge_action(step_name: str,
    ...
    get_state().current_workflow_instance.step_assignments[step_name].remove(int(object_id))
            ...
            with ui.grid(columns=3).classes('w-full items-center gap-4'):
                ...
                with ui.column(align_items='center'):
                    ...
                    target_node_input_add = ui.select(
                        options=[str(obj) for obj in sorted(get_state().existing_objects)])
