...
async def add_edge_action(step_name: str | None,
    ...
    if not step_name or not object_id:
        ...
        return
    elif await workflow_instance_page_state.graph_component.exists_edge(step_name, object_id):
            ...
            with ui.grid(columns=3).classes('w-full items-center gap-4'):
                ...
                with ui.column(align_items='center'):
                    ...
                    target_node_input_add = ui.select(
                        options=[str(obj) for obj in sorted(workflow_instance_page_state.existing_objects)])
