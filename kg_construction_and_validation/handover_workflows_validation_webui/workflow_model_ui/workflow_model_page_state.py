...
class WorkflowModelPageState:
    ...
    def save_workflow_model_copy(self):
        self.workflow_model_history.append((self.selected_node, deepcopy(shared_state().current_workflow_model)))
        self.changes_are_saved = False


    def undo_workflow_model_change(self):
        if len(self.workflow_model_history) > 0:
            self.selected_node, shared_state().current_workflow_model = self.workflow_model_history.pop()